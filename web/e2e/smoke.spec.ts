...
test('Profile preflight pins confirmation tokens and partial operations remain actionable', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    const path = new URL(request.url()).pathname
    if (path === '/api/v1/auth/session') return route.fulfill({ json: session() })
    if (path === '/api/v1/profiles' && request.method() === 'GET') return route.fulfill({ json: { items: [{ id: profileID, name: 'Research', description: 'Pinned fleet profile', revision: 7, skillIds: ['skill-id'], createdAt: now, updatedAt: now }] } })
...
test('Profile list is sourced from migrated Skill-only profiles', async ({ page }) => {
  ...
  const ordinaryProfile = {
    ...
    revision: 2, canonicalHash: '1'.repeat(64), pendingBindings: false, skillIds: [], effectiveVisibleCount: 0,
    createdAt: now, updatedAt: now,
  }
  const legacyProfile = {
    ...ordinaryProfile, id: legacyProfileID, currentRevisionId: 'b4444444-4444-4444-8444-444444444444', name: 'retired-profile-fixture',
  ...
  await page.route('**/api/v1/**', async (route) => {
    const path = new URL(route.request().url()).pathname
    if (path === '/api/v1/auth/session') return route.fulfill({ json: session() })
    if (path === '/api/v1/profiles') return route.fulfill({ json: { items: [ordinaryProfile] } })
    if (path === '/api/v1/skills') return route.fulfill({ json: { items: [] } })
    if (path === '/api/v1/mcp/servers') return route.fulfill({ json: { items: [] } })
...
test('Profile editor is Skill-only and never submits MCP ownership fields', async ({ page }) => {
  const skillID = 'c1111111-1111-4111-8111-111111111111'
  const skillVersionID = 'c2222222-2222-4222-8222-222222222222'
  const profileID = 'c3333333-3333-4333-8333-333333333333'
  const skill = {
    id: skillID,
    slug: 'formatter',
    name: 'Formatter',
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/api/v1/profiles' && request.method() === 'POST') {
      ...
      const profile = {
        ...savedBody,
        id: profileID,
        currentRevisionId: 'c4444444-4444-4444-8444-444444444444',
        revision: 1,
  ...
  await expect(editor.getByText('MCP configuration is managed in the Shared relay editor')).toBeVisible()
  await expect(editor.getByText('MCP tools')).toHaveCount(0)
  await expect(editor.getByText('Tool rules')).toHaveCount(0)
  await editor.getByLabel('Name').fill('skill-only')
  await editor.getByLabel('Client').selectOption({ label: 'Claude' })
  ...
  expect(savedBody).not.toHaveProperty('mcpServerIds')
  expect(savedBody).not.toHaveProperty('mcpRevisionIds')
  expect(savedBody).not.toHaveProperty('mcpGovernance')
  expect(savedBody).not.toHaveProperty('toolRules')
  await expect(page.getByRole('row').filter({ hasText: 'skill-only' })).toContainText('Draft')
})

test('Shared relay editor owns the enabled MCP set and applies through the relay operation', async ({ page }) => {
  const firstServerID = 'd1111111-1111-4111-8111-111111111111'
  const secondServerID = 'd2222222-2222-4222-8222-222222222222'
  const firstRevisionID = 'd3333333-3333-4333-8333-333333333333'
  const secondRevisionID = 'd4444444-4444-4444-8444-444444444444'
  const relayTargetID = 'd5555555-5555-4555-8555-555555555555'
  const operationID = 'd6666666-6666-4666-8666-666666666666'
  const servers = [
    { id: firstServerID, currentRevisionId: firstRevisionID, name: 'acemcp', description: 'Search', revision: 1, transport: 'http', args: [], url: 'http://acemcp.invalid/mcp', envKeys: [], headerKeys: [], contentHash: '1'.repeat(64), enabled: true, createdAt: now, updatedAt: now },
    { id: secondServerID, currentRevisionId: secondRevisionID, name: 'docs', description: 'Docs', revision: 1, transport: 'http', args: [], url: 'http://docs.invalid/mcp', envKeys: [], headerKeys: [], contentHash: '2'.repeat(64), enabled: true, createdAt: now, updatedAt: now },
  ]
  const target = {
    id: relayTargetID,
    targetKey: 'local/shared-relay',
    nodeId: 'd7777777-7777-4777-8777-777777777777',
    nodeName: 'local',
    nodeKind: 'local',
    ...
    health: 'healthy',
    desiredRevision: 1,
    targetRevision: 'relay-target',
    relayMemberStatuses: [{ memberId: firstRevisionID, name: 'acemcp', status: 'ready', capabilityKinds: ['tools'], capabilities: { tools: 2, resources: 0, resourceTemplates: 0, prompts: 0 }, checkedAt: now }],
  }
  const firstRevision = { id: 'd8888888-8888-4888-8888-888888888888', revision: 1, canonicalHash: '3'.repeat(64), mcpServers: [{ serverId: firstServerID, mcpRevisionId: firstRevisionID, position: 0 }], createdAt: now }
  const configuration: Record<string, any> = {
    current: firstRevision,
    applied: firstRevision,
    mode: 'compatibility',
    defaultProfileId: null,
    migration: { state: 'compatibility_ready', pendingContractReviews: 0, ambiguousProfiles: 0, legacyProfileState: 'pending', restorableSnapshot: false },
    runtimeCapability: { compatible: true, runtimeVersion: '2.15.0-toolhub.1', features: [] },
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/api/v1/relay/configuration' && request.method() === 'PUT') {
      ...
      configuration.current = { id: 'd9999999-9999-4999-8999-999999999999', revision: 2, canonicalHash: '4'.repeat(64), mcpServers: [{ serverId: firstServerID, mcpRevisionId: firstRevisionID, position: 0 }, { serverId: secondServerID, mcpRevisionId: secondRevisionID, position: 1 }], createdAt: now }
      return route.fulfill({ json: configuration.current })
    }
    if (path === '/api/v1/relay/configuration/preflight') {
      return route.fulfill({ json: { revisionId: configuration.current.id, routingHash: '5'.repeat(64), result: { targetRevision: 'relay-target-2', manifestHash: '6'.repeat(64), diff: { add: [{ kind: 'mcp', name: 'docs' }], replace: [], delete: [], excluded: [] } } } })
    }
  ...
  await page.goto('/mcp?view=relay')
  await expect(page.getByRole('heading', { name: 'Shared mcpm relay' })).toBeVisible()
  await page.locator('.relay-server-options label').filter({ hasText: 'docs' }).getByRole('checkbox').check()
  await page.getByRole('button', { name: 'Save relay draft' }).click()
  ...
  expect(draftBody).toMatchObject({
    ...
    mcpRevisionIds: { [firstServerID]: firstRevisionID, [secondServerID]: secondRevisionID },
  })
  await page.getByRole('button', { name: 'Run relay preflight' }).click()
  await expect(page.getByText('Add · docs', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'Apply relay update' }).click()
  await expect.poll(() => applyBody).not.toBeNull()
  expect(applyBody).toMatchObject({ revisionId: configuration.current.id, profileIds: [], mode: 'compatibility' })
})
