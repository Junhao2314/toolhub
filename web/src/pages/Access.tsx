...
export default function Access({ currentUserID, sessionInvalidated }: { currentUserID: string; sessionInvalidated: () => void }) {
  ...
  return <>
    <PageHeader title="Users & Audit" detail="Role assignments, temporary credentials, and immutable operational history." actions={<><Button variant="secondary" onClick={state.reload}><RefreshCw size={16} />Refresh</Button>{tab === 'Users' && <Button onClick={() => setAdd(true)}><Plus size={16} />Add user</Button>}</>} />
    <div className="toolbar"><Segments options={['Users', 'Audit']} value={tab} onChange={setTab} /></div>
    {state.loading ? <Loading label="Loading access data" /> : state.error || !state.data ? <ErrorNotice message={state.error} retry={state.reload} /> : tab === 'Users' ? <Users items={state.data.users} reset={setResetUser} /> : <AuditTable items={state.data.audit} />}
...
function UserModal({ close, saved }: { close: () => void; saved: () => void }) {
  ...
  if (created?.temporaryPassword) return <TemporaryPasswordModal title={`Temporary password for @${created.user.username}`} password={created.temporaryPassword} close={close} />
  return <Modal title="Add user" close={close}>{error && <ErrorNotice message={error} />}<Field label="Display name"><input value={displayName} onChange={(event) => setName(event.target.value)} /></Field><Field label="Username" hint="3–32 lowercase letters, numbers, dots, underscores, or hyphens"><input autoCapitalize="none" minLength={3} maxLength={32} pattern="[A-Za-z0-9._-]+" value={username} onChange={(event) => setUsername(event.target.value)} /></Field><Field label="Email"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></Field><PasswordMode value={passwordMode} setValue={setPasswordMode} password={password} setPassword={setPassword} /><Field label="Role"><select value={role} onChange={(event) => setRole(event.target.value)}><option value="viewer">Viewer</option><option value="operator">Operator</option><option value="admin">Admin</option></select></Field><div className="role-explainer"><Shield size={18} /><span><strong>{role}</strong>{role === 'admin' ? ' can manage users, credentials, and approvals.' : role === 'operator' ? ' can manage nodes, skills, jobs, and MCP.' : ' has read-only visibility.'}</span></div><div className="modal-actions"><Button variant="secondary" onClick={close}>Cancel</Button><Button onClick={submit} disabled={busy || !displayName || username.length < 3 || !email || (passwordMode === 'manual' && password.length < 12)}><UserCog size={15} />{busy ? 'Creating...' : 'Create user'}</Button></div></Modal>
}
...
function PasswordModal({ user, isSelf, close, saved, sessionInvalidated }: { user: User; isSelf: boolean; close: () => void; saved: () => void; sessionInvalidated: () => void }) {
  ...
  if (temporaryPassword) return <TemporaryPasswordModal title={`Temporary password for @${user.username}`} password={temporaryPassword} close={finish} />
  return <Modal title={`Reset password for ${user.displayName}`} close={close}>{error && <ErrorNotice message={error} />}<p className="inline-notice">Every active session for this user will be signed out immediately.</p><PasswordMode value={mode} setValue={setMode} password={password} setPassword={setPassword} /><div className="modal-actions"><Button variant="secondary" onClick={close}>Cancel</Button><Button onClick={submit} disabled={busy || (mode === 'manual' && password.length < 12)}><KeyRound size={15} />{busy ? 'Resetting...' : 'Reset password'}</Button></div></Modal>
}
...
function PasswordMode({ value, setValue, password, setPassword }: { value: string; setValue: (value: string) => void; password: string; setPassword: (value: string) => void }) {
  return <div className="credential-mode"><strong>Temporary password</strong><label><input type="radio" name="password-mode" checked={value === 'random'} onChange={() => setValue('random')} />Generate a secure random password</label><label><input type="radio" name="password-mode" checked={value === 'manual'} onChange={() => setValue('manual')} />Set a temporary password manually</label>{value === 'manual' && <Field label="Temporary password"><input type="password" minLength={12} value={password} onChange={(event) => setPassword(event.target.value)} /></Field>}</div>
}
...
function TemporaryPasswordModal({ title, password, close }: { title: string; password: string; close: () => void }) {
  ...
  return <Modal title={title} close={close}><p className="inline-notice">This password is shown once. Copy it before closing.</p><code className="temporary-password">{password}</code><div className="modal-actions"><Button variant="secondary" onClick={copy}><Clipboard size={15} />{copied ? 'Copied' : 'Copy password'}</Button><Button onClick={close}>Done</Button></div></Modal>
}
