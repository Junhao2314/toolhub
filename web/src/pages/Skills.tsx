import { Archive, Check, Download, FileUp, GitBranch, Import, RefreshCw, RotateCcw, Search, ShieldAlert, WandSparkles } from 'lucide-react'
...
interface Discovery { id: string; kind: string; nodeName: string; runtime: string; name: string; path: string; sha256: string; managed: boolean; protected: boolean; missing: boolean; drift: boolean; status: string; lastError: string; controlMode: string; sourceChanged: boolean; importStatus: string; linkCoverage?: Record<string, string> }
...
export default function Skills({ canAdopt }: { canAdopt: boolean }) {
  ...
  const provenanceOptions = [...new Set((state.data?.skills ?? []).map((skill) => skill.sourceKind || 'upload'))].sort()
  const filtered = (state.data?.skills ?? []).filter((skill) => `${skill.name} ${skill.description} ${skill.slug}`.toLowerCase().includes(query.toLowerCase()) && (view !== 'Review' || skill.reviewStatus === 'pending') && (provenance === 'all' || (skill.sourceKind || 'upload') === provenance))
  return <>
...
function DiscoveredTable({ items, busy, canAdopt, adopt, importSnapshot }: { items: Discovery[]; busy: string; canAdopt: boolean; adopt: (id: string) => void; importSnapshot: (item: Discovery) => void }) {
  const { t } = useI18n()
  ...
  return <div className="table-scroll"><table><thead><tr><th>{t('Skill')}</th><th>{t('Node / runtime')}</th><th>{t('Hash')}</th><th>{t('State')}</th><th /></tr></thead><tbody>{items.map((item) => { const hermes = item.runtime === 'hermes' || item.controlMode === 'read_only_source'; const importing = item.importStatus === 'queued' || item.importStatus === 'importing'; const state = item.missing ? 'missing' : item.sourceChanged ? 'source changed' : item.drift ? 'drift' : item.status; return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.path}</small>{item.runtime === 'shared' && <small>{t('Consumer links: {coverage}', { coverage: Object.entries(item.linkCoverage ?? {}).map(([runtime, state]) => `${runtime}:${state}`).join(', ') || t('not reported') })}</small>}</td><td>{item.nodeName}<small>{item.runtime === 'shared' ? t('Canonical shared source') : hermes ? t('Hermes read-only source') : item.runtime}</small></td><td><code>{item.sha256?.slice(0, 12) || t('unavailable')}</code></td><td><Status value={state} />{item.lastError && <small>{item.lastError}</small>}</td><td className="row-actions">{canAdopt && (hermes ? <Button variant="secondary" disabled={busy === item.id || importing || item.protected || item.missing || !item.sha256} onClick={() => importSnapshot(item)}><Import size={15} />{t(item.importStatus === 'imported' || item.sourceChanged ? 'Re-import' : 'Import snapshot')}</Button> : <Button variant="secondary" disabled={busy === item.id || item.managed || item.protected || item.missing || !item.sha256} onClick={() => adopt(item.id)}><WandSparkles size={15} />{t('Adopt')}</Button>)}</td></tr> })}</tbody></table></div>
}
...
function TargetModal({ skill, close, saved }: { skill: Skill; close: () => void; saved: () => void }) {
  const { t } = useI18n()
  type Activation = { runtime: string; profileName: string }
  type TargetNode = { id: string; name: string; status: string; isLocal: boolean; runtimeKinds: string[]; activations: Activation[] }
  ...
  useEffect(() => {
    ...
    if (!local) return
    const defaults = local.runtimeKinds.filter((runtime) => runtime !== 'shared' && runtime !== 'hermes' && !local.activations?.some((activation) => activation.runtime === runtime))
  ...
  const submit = () => {
    const managed = new Set((nodes.data?.items ?? []).flatMap((node) => (node.activations ?? []).map((activation) => `${node.id}:${activation.runtime}`)))
    const matrix = Object.entries(targets).filter(([key, enabled]) => enabled && !managed.has(key)).map(([key]) => { const [nodeId, runtime] = key.split(':'); return { nodeId, runtime, enabled: true } })
