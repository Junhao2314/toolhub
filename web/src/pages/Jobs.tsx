...
export default function Jobs() {
  ...
  return <>
    <PageHeader title={t('Jobs')} detail={t('Scheduled and manual update, sync, inventory, MCP, and rollback work.')} actions={<Button variant="secondary" onClick={state.reload}><RefreshCw size={16} />{t('Refresh')}</Button>} />
    {error && <ErrorNotice message={error} />}
    <div className="toolbar"><Segments options={['All', 'Pending', 'Running', 'Failed', 'Succeeded']} value={filter} onChange={setFilter} /></div>
    {state.loading ? <Loading label={t('Loading job queue')} /> : state.error ? <ErrorNotice message={state.error} retry={state.reload} /> : jobs.length === 0 ? <Empty title={t('No jobs in this view')} detail={t('Scheduled checks and manual operations appear here.')} /> : <div className="table-scroll"><table><thead><tr><th>{t('Job')}</th><th>{t('Mode')}</th><th>{t('Created')}</th><th>{t('Attempts')}</th><th>{t('Status')}</th><th /></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td><strong>{job.kind.replaceAll('_', ' ')}</strong><small>{job.id.slice(0, 12)}</small></td><td>{job.dryRun ? t('Dry run') : t('Apply')}</td><td>{new Date(job.createdAt).toLocaleString()}</td><td>{job.attempts}/{job.maxAttempts}</td><td><Status value={job.status} /></td><td>{['pending', 'running'].includes(job.status) && <IconButton label={t('Cancel job')} onClick={() => cancel(job.id)}><Ban size={16} /></IconButton>}</td></tr>)}</tbody></table></div>}
  </>
}
