...
interface TargetView {
  ...
  runtime: string
  capabilities: { skills: boolean; mcp: boolean; readOnly: boolean; mcpNote: string }
...
function TargetDetail({ nodeID, runtime, canOperate }: { nodeID: string; runtime: string; canOperate: boolean }) {
  ...
  const visibleSkills = target.capabilities.readOnly ? target.skills : target.skills.filter((skill) => skill.desiredEnabled)
  return <div className="runtime-view">
    {error && <ErrorNotice message={error} />}
    {notice && <div className="inline-notice">{notice}</div>}
    <section className="runtime-band activation-band"><header><div><MonitorCog size={19} /><span><h2>{target.capabilities.readOnly ? t('Read-only runtime source') : t('Active Profile')}</h2><p>{target.capabilities.readOnly ? t('Observed state is never a ToolHub deployment target.') : t('Desired state ownership for this node and runtime.')}</p></span></div><span className="drift-summary"><Status value={target.capabilities.readOnly ? 'read only' : target.activation?.state ?? 'manual'} /><IconButton label={t('Refresh Runtime View')} onClick={state.reload}><RefreshCw size={16} /></IconButton></span></header>{target.activation ? <div className="activation-detail"><dl><div><dt>{t('Profile')}</dt><dd>{target.activation.profileName}</dd></div><div><dt>{t('Activated by')}</dt><dd>{target.activation.activatedBy || t('system')}</dd></div><div><dt>{t('Activated at')}</dt><dd>{new Date(target.activation.activatedAt).toLocaleString()}</dd></div><div><dt>{t('Previous Profile')}</dt><dd>{target.activation.previousProfileName || '—'}</dd></div></dl>{target.activation.skipped?.length > 0 && <div className="runtime-note"><strong>{t('Skipped during activation')}</strong>{target.activation.skipped.map((issue, index) => <span key={`${issue.reason}-${index}`}>{issue.reason}<small>{issue.detail}</small></span>)}</div>}{target.activation.lastError && <ErrorNotice message={target.activation.lastError} />}{canOperate && <div className="runtime-actions"><Button variant="secondary" disabled={busy !== ''} onClick={() => queueProfile(target.activation!.profileId, target.activation!.profileName)}><RotateCw size={16} />{t('Retry activation')}</Button>{target.activation.previousProfileId && <Button variant="secondary" disabled={busy !== ''} onClick={() => queueProfile(target.activation!.previousProfileId, target.activation!.previousProfileName)}><ArrowLeftRight size={16} />{t('Return to {profile}', { profile: target.activation.previousProfileName })}</Button>}<Button variant="danger" disabled={busy !== ''} onClick={deactivate}><ShieldOff size={16} />{t('Deactivate Profile')}</Button></div>}</div> : target.capabilities.readOnly ? <div className="manual-control read-only-control"><strong>{t('Observation and explicit import only')}</strong><span>{t('Source changes are reported here without drift, synchronization, or writes back to Hermes.')}</span></div> : <div className="manual-control"><strong>{t('Manual target control')}</strong><span>{t('No Profile owns this target. Skills and MCP matrices can be edited directly.')}</span></div>}</section>
    <section className="runtime-band"><header><div><span><h2>{target.capabilities.readOnly ? t('Observed MCP servers') : t('Effective MCP servers')}</h2><p>{target.mcp.mcpmProfile || t('Observed runtime configuration')}</p></span></div><span className="drift-summary"><strong>{target.mcp.servers.length}</strong><small>{t('servers')}</small><Status value={target.drift.mcp ? 'drift' : target.mcp.state} /></span></header>{!target.capabilities.mcp && target.capabilities.mcpNote && <div className="inline-notice">{target.capabilities.mcpNote}</div>}{target.mcp.servers.length ? <div className="table-scroll"><table><thead><tr><th>{t('Server')}</th><th>{t('Transport')}</th><th>{t('Endpoint')}</th><th>{t('Source')}</th><th>{t('State')}</th></tr></thead><tbody>{target.mcp.servers.map((server) => <tr key={server.id}><td><strong>{server.name}</strong><small>{server.runtimeName}{server.observedGeneration ? ` · ${t('generation {n}', { n: server.observedGeneration })}` : ''}</small></td><td>{server.transport}</td><td><code>{server.endpoint}</code></td><td>{server.originName || server.source}</td><td><Status value={server.missing ? 'missing' : server.sourceChanged ? 'source changed' : server.drift ? 'drift' : server.readOnly ? 'read only' : 'in sync'} /></td></tr>)}</tbody></table></div> : <Empty title={target.capabilities.readOnly ? t('No observed MCP servers') : t('No effective MCP servers')} detail={target.capabilities.readOnly ? t('No Hermes MCP entries were present in the latest inventory.') : t('This target currently has an empty MCP selection.')} />}</section>
    <section className="runtime-band"><header><div><span><h2>{target.capabilities.readOnly ? t('Observed Skills') : t('Effective Skills')}</h2><p>{target.capabilities.readOnly ? t('Hermes filesystem snapshots available for explicit import.') : t('Enabled desired deployments for this runtime.')}</p></span></div><span className="drift-summary"><strong>{visibleSkills.length}</strong><small>{t('Skills')}</small><Status value={target.capabilities.readOnly ? 'read only' : target.drift.skills ? 'drift' : 'in sync'} /></span></header>{visibleSkills.length ? target.capabilities.readOnly ? <div className="table-scroll"><table><thead><tr><th>{t('Skill')}</th><th>{t('Hash')}</th><th>{t('Path')}</th><th>{t('Import')}</th><th>{t('State')}</th></tr></thead><tbody>{visibleSkills.map((skill) => <tr key={skill.discoveryId || skill.skillId}><td><strong>{skill.name}</strong><small>{skill.slug}</small></td><td><code>{skill.sha256?.slice(0, 12) || '—'}</code></td><td><code>{skill.path || '—'}</code></td><td><Status value={skill.importStatus || 'available'} /></td><td><Status value={skill.missing ? 'missing' : skill.sourceChanged ? 'source changed' : 'read only'} />{skill.lastError && <small>{skill.lastError}</small>}</td></tr>)}</tbody></table></div> : <div className="table-scroll"><table><thead><tr><th>{t('Skill')}</th><th>{t('Hash')}</th><th>{t('Desired')}</th><th>{t('Actual')}</th><th>{t('State')}</th><th>{t('Last error')}</th></tr></thead><tbody>{visibleSkills.map((skill) => { const drift = skill.desiredEnabled !== skill.actualEnabled || skill.desiredVersionId !== skill.actualVersionId; return <tr key={skill.deploymentId}><td><strong>{skill.name}</strong><small>{skill.slug}</small></td><td><code>{skill.sha256?.slice(0, 12) || '—'}</code></td><td>{skill.desiredEnabled ? t('Enabled') : t('Disabled')}</td><td>{skill.actualEnabled ? t('Enabled') : t('Disabled')}</td><td><Status value={drift ? 'drift' : skill.state} /></td><td>{skill.lastError || '—'}</td></tr> })}</tbody></table></div> : <Empty title={target.capabilities.readOnly ? t('No observed Skills') : t('No effective Skills')} detail={target.capabilities.readOnly ? t('No Hermes Skills were present in the latest inventory.') : t('This target currently has an empty Skill selection.')} />}</section>
