...
func (s *Store) ListNodes(ctx context.Context) (json.RawMessage, error) {
	return s.JSONList(ctx, `SELECT n.id::text AS id,n.name,n.hostname,n.platform,n.architecture,host(n.tailscale_ip) AS "tailscaleIp",n.status,n.labels,
		...
		coalesce((SELECT jsonb_agg(jsonb_build_object('runtime',a.runtime_kind,'profileId',a.profile_id::text,'profileName',p.name,'state',a.state)
			ORDER BY a.runtime_kind) FROM toolhub_profile_activations a JOIN toolhub_profiles p ON p.id=a.profile_id
			WHERE a.node_id=n.id AND a.state IN ('pending','active','partial','failed')),'[]'::jsonb) AS activations
...
func (s *Store) ListMCPServers(ctx context.Context) (json.RawMessage, error) {
	return s.JSONList(ctx, `SELECT id::text AS id,name,runtime_name AS "runtimeName",transport,command,args,url,
		coalesce((SELECT jsonb_agg(key ORDER BY key) FROM jsonb_object_keys(env_refs) key),'[]'::jsonb) AS "envKeys",
		coalesce((SELECT jsonb_agg(key ORDER BY key) FROM jsonb_object_keys(header_refs) key),'[]'::jsonb) AS "headerKeys",enabled,source,origin,
